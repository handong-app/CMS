...
async function run() {
  try {
    ...
    const coderabbitaiApproval = reviews.data.find(
      (review) =>
    ...
    if (!coderabbitaiApproval) {
      core.info(
        "coderabbitai[bot] has not approved this PR. Skipping reviewer assignment."
      );
      return;
    }

    core.info(
      "coderabbitai[bot] has approved the PR. Proceeding with reviewer assignment."
    );
